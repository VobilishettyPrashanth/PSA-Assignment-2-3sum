...
public class ThreeSumBenchmark {
    ...
    private void benchmarkThreeSum(final String description, final Consumer<int[]> function, int n, final TimeLogger[] timeLoggers) {
        if (description.equals("ThreeSumCubic") && n <=16000) {
            long startTime=System.currentTimeMillis();
            function.accept(supplier.get());
            long endTime=System.currentTimeMillis();
            System.out.println("Raw time per each run (in mSec): "+n+" is "+ (endTime-startTime));
            System.out.println("Normalized time per each run (n^3): "+n+" is "+ (endTime-startTime)*1.0/ n / n / n * 1e6);

        }
        // FIXME
        else if(description.equals("ThreeSumQuadratic") && n <= 16000) {
            long startTime=System.currentTimeMillis();
            function.accept(supplier.get());
            long endTime=System.currentTimeMillis();
            System.out.println("Raw time per each run (in mSec): "+n+" is "+ (endTime-startTime));
            System.out.println("Normalized time per each run (n^2): "+n+" is "+ (endTime-startTime)*1.0/ n / n * 1e6);

        }
        else if(description.equals("ThreeSumQuadrithmic") && n <= 16000) {
            long startTime=System.currentTimeMillis();
            function.accept(supplier.get());
            long endTime=System.currentTimeMillis();
            System.out.println("Raw time per each run (in mSec): "+n+" is "+ (endTime-startTime));
            System.out.println("Normalized time per each run (n^2 log n): "+n+" is "+ (endTime-startTime)*1.0/ n / n / Utilities.lg(n) * 1e6);
        }
    ...
    private final static TimeLogger[] timeLoggersCubic = {
            new TimeLogger("Raw time per each run (in mSec): ", (time, n) -> time),
            new TimeLogger("Normalized time per each run (n^3): ", (time, n) -> time / n / n / n * 1e6)
    };
    private final static TimeLogger[] timeLoggersQuadrithmic = {
            new TimeLogger("Raw time per each run (in mSec): ", (time, n) -> time),
            new TimeLogger("Normalized time per each run (n^2 log n): ", (time, n) -> time / n / n / Utilities.lg(n) * 1e6)
    };
    private final static TimeLogger[] timeLoggersQuadratic = {
            new TimeLogger("Raw time per each run (in mSec): ", (time, n) -> time),
            new TimeLogger("Normalized time per each run (n^2): ", (time, n) -> time / n / n * 1e6)
    };
